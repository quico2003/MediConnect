...
import OrganisationCard from "../Components/OrganisationPanel/OrganisationCard";

const { maxResults } = Configuration.search.organisations;

const OrganisationPanel = () => {
  const { strings: Strings } = useContext(StringsContext);
  const ViewStrings = Strings.Search.Organisations;
  ...
  const fetchData = () => {
    ...
    request("get", getEndpoint(Endpoints.Search.organisations.get), {
      search,
      offset: maxResults,
    })
      .then((res) => {
        ...
        <Row>
          {data.map((org) => (
            <Col xs={12} sm={6} xl={4} xxl={3}>
              <OrganisationCard key={org.guid} {...org} />
            </Col>
          ))}
        </Row>
      )}
